...
class Wings_Of_Justice:
    ...
    def setup(self):
        ...
        if self.agent.onSurface:
            ...
            if delta_a.magnitude() < 1000:
                if abs(self.drive_controls.steer) <= 0.15:# or self.agent.team == 1 and self.agent.currentSpd < 300: #self.agent.currentSpd < 300 or
    ...
    def update(self):
        ...
        boost_req = clamp(8,1,abs(8-clamp(8,0,self.boostHistory)))*self.agent.aerialAccelerationTick
        #boost_req = self.agent.aerialAccelerationTick
        ...
        if dt >1 or delta_a.magnitude() > boost_req:
            ...
            if delta_a.magnitude() > 2 and aligned:
                controls.throttle = 1
            if delta_a.magnitude() > 1060 and dt > 1:
                self.active = False
                self.agent.log.append(f"required acceleration too high {delta_a.magnitude()}")
        else:
            align_car_to(controls, self.agent.me.avelocity, (self.agent.me.location - self.pred.location).normalize(), self.agent)
    ...
    def update(self):
        ...
        if jump:
            if self.targetCode == 1:
                ...
                # diagnal flip cancel
                controller_state.pitch = -.2
                controller_state.yaw = -.8
                # controller_state.steer = -1
                controller_state.throttle = 1
...
class Divine_Mandate:
    ...
    def create_custom_controls(self, actionCode):
        ...
        controller_state = SimpleControllerState()
        if actionCode == 0:
            if butterZone(self.agent.ball.location) and sign(self.agent.team) * self.agent.ball.location[1] < 0:
                ball_local = localizeVector(self.agent.ball.location + Vector([0,0,20]),self.agent.me)
            else:
                ball_local = self.agent.ball.local_location
            ball_angle = math.atan2(ball_local.data[1], ball_local.data[0])
            controller_state.jump = True
...
class PreemptiveStrike(baseState):
    ...
    def create_speed_flip_cancel(self,left=False):
        ...
        timers.append(self.agent.fakeDeltaTime*3)
    ...
    def setup(self):
        ...
        #setup flips
        ball_local = self.agent.ball.local_location #localizeVector(Vector([0, 0, 0]), self.agent.me)
        if self.kickoff_type == 0:
            if ball_local[1] > 0:
    ...
    def wide_handler(self):
        ...
        if self.phase == 1:
            if distance2D(self.agent.me.location,self.agent.ball.location) > 2950:
                #return driveController(self.agent, self.enemyGoal+Vector([0,4000*sign(self.agent.team),0]),
                return driveController(self.agent, self.agent.ball.location+Vector([0,sign(self.agent.team)*350,0]),
                                                  0,
                                                  expedite=True, flippant=False,
                                                  maintainSpeed=self.maintaining_speed,kickoff=True)
            else:
                self.phase = 3
        ...
        if self.phase == 3:
            ...
            if not self.agent.onSurface:
                ...
                _controller.steer, _controller.yaw, _controller.pitch, _controller.roll,_err = point_at_position(self.agent,self.agent.ball.location)
        ...
        if self.phase == 4:
            if distance2D(self.agent.me.location,self.agent.ball.location) > 450:
                if self.agent.me.location[0] > self.agent.ball.location[0]:
                    dummy_location = self.agent.ball.location + Vector([30, 75*sign(self.agent.team), 0])
        ...
        if self.phase == 5:
            if self.secondFlip == None:
                if self.agent.team == 0 and self.agent.me.location[0] < self.agent.ball.location[0] or self.agent.team == 1 and self.agent.me.location[0] > self.agent.ball.location[0]:
                    _code = 4
                else:
                    _code = 5
    ...
    def short_handler(self):
        # stage 1 - drive to boost pad
        if self.phase == 1:
            drive_target = Vector([self.short_offset, sign(self.agent.team)*2825.0,0])
        ...
        # stage 4 - aim towards just offcenter of ball
        if self.phase == 4:
            if distance2D(self.agent.me.location, self.agent.ball.location) > 500 or not self.agent.onSurface:
                if self.agent.me.location[0] > self.agent.ball.location[0]:
                    dummy_location = self.agent.ball.location + Vector([65, 75*sign(self.agent.team), 0])
                    #_direction = direction(self.enemyGoal, self.agent.ball.location)
                    drive_target = dummy_location #+ _direction.scale(
                        #distance2D(self.agent.me.location, dummy_location) * .5)
                else:
        ...
        if self.phase == 5:
            if self.secondFlip == None:
                if self.agent.team == 0 and self.agent.me.location[0] < self.agent.ball.location[
                    0] or self.agent.team == 1 and self.agent.me.location[0] > self.agent.ball.location[0]:
                    _code = 4
                else:
                    _code = 5
    ...
    def middle_handler(self):
        ...
        if self.phase == 4:
            if distance2D(self.agent.me.location, self.agent.ball.location) > 500 or not self.agent.onSurface:
                if self.agent.me.location[0] > self.agent.ball.location[0]:
                    dummy_location = self.agent.ball.location + Vector([55, 75*sign(self.agent.team), 0])
                    #_direction = direction(self.enemyGoal, self.agent.ball.location)
                    drive_target = dummy_location #+ _direction.scale(
                        #distance2D(self.agent.me.location, dummy_location) * .5)
                else:
...
class Do_No_Evil(baseState):
    ...
    def update(self):
        ...
        elif (self.agent.scorePred.time - self.agent.time)+0.5 > self.agent.enemyBallInterceptDelay:
            self.active = False
        print(f"doing no evil! {self.agent.time}")
            ...
            if man == 2:
                if agent.lastMan != agent.me.location:
                    if hit.pred_vector[1]* -sign(agent.team) > 4000 * -sign(agent.team):
                        if butterZone(hit.pred_vector):
                            if hit.time_difference() < agent.enemyBallInterceptDelay or (hit.time_difference() < 2 and agent.lastMan[1] * sign(agent.team) > 0):
                                man = 1
                                agent.log.append(
    ...
    if agentType != PreemptiveStrike:
        ...
        if not kickOffTest(agent):
            ...
            if agent.scorePred != None:
                if clamp(15000,0.0001,findDistance(agent.closestEnemyToBall.location,agent.scorePred.location)-150)/2300 > agent.scorePred.time-agent.time:
                    if (agent.scorePred.time - agent.time)+0.5 < agent.enemyBallInterceptDelay:
                        #if agent.team == 1:
