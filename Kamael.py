...
class Kamael(BaseAgent):
    ...
    def preprocess(self, game):
        ...
        if not car.is_demolished:
            ...
            self.me.retreating = player_retreat_status(self.me, self.ball.location,self.team,num_allies = ally_count)
        ...
        if not self.hitbox_set:
            ...
            )*0.9 # (92 + (car.hitbox.length * 0.665)) * 0.9
        ...
        for i in range(game.num_cars):
            if i != self.index:
                ...
                if not car.is_demolished:
                    ...
                    if car.team == self.team:
                        _obj.retreating = player_retreat_status(_obj, self.ball.location,car.team,num_allies = ally_count)
        ...
        if len(self.allies) < 1 and self.team == 3:
            ...
            if len(self.hits) < 1:
                ...
                if updateHits == True or self.sorted_hits[0].prediction_time < self.time or not predictions_valid or self.time - self.update_time > refresh_timer:
                    self.update_hits()
    ...
    def createJumpChain(self, timeAlloted, targetHeight, jumpSim = None,set_state = True):
        ...
        if jumpSim[-1] == False:
            ...
            if targetHeight >= self.double_point_limit:
                controls.append(1)
                timers.append(clamp(2,0,timeRemaining+self.fakeDeltaTime*20))
                #print("in here")
    ...
    def update_hits(self):
        ...
        self.hits = findHits(
            self, self.groundCutOff, self.singleJumpLimit, self.doubleJumpLimit,resolution=res)
