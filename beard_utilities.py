...
def add_car_offset(agent, projecting=False):
    ...
    #if agent.team == 1:
    #agent.me.location = agent.me.location + agent.me.velocity.scale(1/120)
...
def gate(agent,hurry=True):
    ...
    selectedDist = 99999
    if agent.goalie and agent.lastMan != agent.me.location and abs(agent.me.location[0]) < 800:
...
def ShellTime(agent, retreat_enabled = True):
    ...
    if len(agent.allies) < 2 and agent.team == 0 or len(agent.allies) < 1:
    ...
    if offensive and relativeSpeed(agent.currentHit.pred_vel,agent.me.velocity) > distance2D(agent.me.location,attackTarget)*0.6 and agent.ballDelay < 0.6:
        return handleBounceShot(agent, waitForShot=False)
    ...
    if not destination:
        if agent.contested and agent.enemyBallInterceptDelay <= agent.currentHit.time_difference():
...
def prototype_rotations(agent):
    ...
    for ally in allies:
        relavent_data.append([distance2D(ally.location,agent.ball.location),ally.location[1]*sign(ally.team) >= agent.ball.location[1] * sign(ally.team) or agent.ball.location[1] * sign(agent.team) < 4600,ally])
    ...
    def find_next_man(ignore_positioning = False,ignore_retreating = False):
        ...
        for ally in relavent_data:
            ...
            if not ignore_retreating and ally[2].retreating and agent.ball.location[1] * sign(agent.team) < 4600:
                continue
...
def playBack(agent, buffer=4500, get_boost=True):
    ...
    # if agent.team == 0:
    #     centerField.data[1] = agent.currentHit.pred_vector.flatten()[1] + sign(agent.team)*buffer
    ...
    #     else:
    #         x_addition = -500
    #     centerField = Vector([agent.ball.location[0]+x_addition,agent.ball.location[1] +(sign(agent.team)*buffer),0])
...
def lineupShot(agent, multi):
    ...
    if not targetLoc:
        if agent.forward:
            if sign(agent.team) * targetVec[1] <= 0:# or agent.openGoal: # or (agent.team == 0 and not agent.contested): #and agent.enemyBallInterceptDelay > 1:
                multiCap = clamp(0.4, 0.3, distance / 10000)
                #print("in second shot")
                multi = clamp(
                    multiCap, 0.2, (5000 - abs(agent.me.location[0])) / 10000
                )
                positioningOffset = clamp(
...
def get_aim_vector(agent,end_target_vec,target_ball_vec,target_ball_velocity,offset_length):
    ...
    if agent.team == 1 and target_ball_vec[1] < -10:
...
def dirtyCarryCheck(agent):
    #if agent.team == 0:
    maxRange = 145
    acceptableDistance = findDistance(agent.me.location, agent.ball.location) <= maxRange
    # else:
    #     maxRange = 125
                    ...
                    if agent.time - agent.touch.time_seconds < 1:
                        if acceptableDistance:
                            if (
                                ...
                                >= ballRadius + agent.defaultElevation*.5
...
def handleWallShot(agent):
    ...
    return driveController(agent,destination,agent.currentHit.time_difference(),expedite=True)
...
def driveController(
    agent, target, arrivalTime, expedite=False, flippant=False, maintainSpeed=False,flips_enabled=True,kickoff=False
):
    if agent.boostMonster:
        expedite = True
        flips_enabled = False
    ...
    boost_req = (clamp(8, 1, abs(8 - clamp(8, 0, agent.boost_counter))) * agent.boostAccelerationRate)/120
    ...
    if agent.onWall and target[2] <= agent.defaultElevation:
        ...
        if abs(wall_angle_degrees) < 5:
            if agent.me.location[2] < 2500 and agent.me.location[2] > 120:
                agent.setJumping(-1)
                print("jumpingoff wall!")
...
def rockSteer(angle, distance, forward=True, modifier=600):
    turn = Gsteer(angle)
...
def findHits(agent, grounder_cutoff, jumpshot_cutoff, doubleCutOff,resolution=3):
    ...
    for i in range(0, agent.ballPred.num_slices):
        ...
        if aerialShot == None:
            if agent.aerialsEnabled and ((agent.me.location != agent.lastMan or not agent.onSurface)or len(agent.allies) > 1):
                if not checkAngles or (
                        checkAngles and is_shot_scorable(pred_vec, leftPost, rightPost)[
                    2]):
                    """
                    ...
                    if agent.me.boostLevel >= 1: #and agent.me.location[1] * sign(agent.team) > pred_vec[1] * sign(
                        ...
                        if agent.me.velocity[2] > -100 or pred_vec[2] < agent.me.location[2]:
                            if (inaccurateArrivalEstimator(agent, pred_vec, False, offset=_offset)< tth):
                                ...
                                if not defensiveTouch:
                                    if butterZone(pred_vec):
                                        target = get_aim_vector(agent, center.flatten(), pred_vec,
                                                       pred_vel, agent.reachLength*.9)[0]
                                    else:
                                        _direction = direction(pred_vec.flatten(),center.flatten()).normalize()
                                        target = pred_vec+_direction.scale(agent.reachLength*.95)
                                    ...
                                    target = pred_vec+_direction.scale(agent.reachLength*.9)
                                ...
                                if agent.onSurface:
                                    if enough_takeoff_room(agent,pred_vec)[0] or agent.onWall:
                                        if pred_vec[2] > doubleCutOff+100: #and tth < agent.enemyBallInterceptDelay:
                                            if agent.me.location[1] * sign(agent.team) > pred_vec[1] * sign(agent.team):

                                                accel_req_limit = clamp(1000,750,1000 - (((6-tth)*0.25))*100)
                                                delta_a = calculate_delta_acceleration(target - agent.me.location,
                                                                                       agent.me.velocity + agent.up.scale(300),
                                                                                       tth, agent.gravity)
                                                if delta_a.magnitude() < accel_req_limit:
                                    ...
                                    if pred_vec[2] > jumpshot_cutoff or tth <= 1.5:
                                        delta_a = calculate_delta_acceleration(target - agent.me.location,
                                                                               agent.me.velocity,
                                                                               tth, agent.gravity)
                                        accel_req_limit = clamp(1000, 775, 1000 - (((6 - tth) * .15))*100)
...
def new_ground_wall_estimator(agent,destination):
    offset = agent.reachLength
    if agent.onWall:
        flattened_destination = unroll_path_from_wall_to_ground(agent.me.location,destination)
    else:
